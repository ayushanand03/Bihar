...
# Allow user to select whether to display all entries
display_all = st.checkbox('Display All Entries')
...
if display_all:
    # Plot a grouped bar chart for population comparison for all entries
    fig, ax = plt.subplots(figsize=(12, 6))
    width = 0.4

    ax.bar(np.arange(2) - width/2, data['Population'][0], width, label='Hindu', color='orange')
    ax.bar(np.arange(2) + width/2, data['Population'][1], width, label='Muslim', color='#aaffaa')

    ax.set_xlabel('Religion', fontsize=12)
    ax.set_ylabel('Population', fontsize=12)
    ax.set_title('Population Comparison for All Entries', fontsize=14)
    ...
    # Allow user to select a specific location
